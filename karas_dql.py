...
from DeepLearningPython35.keras_network import Network_gpu
...
def run():
    ...
    dql = Network_gpu()  # a four layer network, with last layer is output
    Q = 0  # Q value
    epochs = 20
    simu_steps = 0
    N = 300
    ...
    for M in range(epochs):
        ...
        num_epoch += 1
        print('start training %dth eposide' % num_epoch)
        ...
        while int(traci.simulation.getCurrentTime()*0.001) < N:
            ...
            if int(traci.simulation.getCurrentTime()*0.001) - last_phase_change >= min_phase_time:
                ...
                if simu_steps < somtthing:  # TODO:
                    for i in range(2):
                        print(transform_x(next_state, [i]).shape)
                        NextQ.append(dql.feedforward(transform_x(next_state, [i])))
                    print(NextQ)
                ...
                # replay!!!!!!!!
                dql.fetch_state(transform(cur_state, action, Q))
