...
def generate_var():
    ...
    #     ran_list.append(rnd_array)
    c = np.linspace(15, 24, 10)
    c = [11, 12, 13, 14]
    # c = [8, 9, 10]
    ac = []
    for i in c:
    ...
    lis = []
    for i in ac:
        for b in range(100-2*i):
            if b+1 > i:
                d = 100 - 2*i - b - 1
                if d > i:
                    lis.append((i, b+1, i, d))
...
def run():
    ...
    for a, b, c, d in list:
        ...
        while int(traci.simulation.getCurrentTime()*0.001) < N + 300:
            ...
            if int(traci.simulation.getCurrentTime()*0.001) - last_phase_change >= min_phase_time:
                if yellow:
                    ...
                    next_phase = logic.get_phase(current_phase, a, b, c)
        ...
        traci.close()
        time, num = cal_average_time()
        results.append(time/num)
        sys.stdout.flush()
    print('the smallest time is: ', min(results),
          'which is from ', list[results.index(min(results))])
