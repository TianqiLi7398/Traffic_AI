...
class Network(object):
    ...
    def get_action(self, state):
        ...
        q_values = []
        for i in range(2):
            q_values.append(np.max(
                self.feedforward(state+[i, 1])))
    ...
    def update_mini_batch(self, mini_batch, eta):
        ...
        for x_, reward, next_state in mini_batch:
            delta_nabla_b, delta_nabla_w = self.backprop(
                x_, reward, next_state)
            nabla_b = [
    ...
    def replay(self, reward, next_state):
        # y_j = r_j + gamma * max(Q(s_j+1 , a))
        Q_value = []
        for i in range(2):
            Q_value.append(np.max(
                self.feedforward(next_state + [i, 1])))
        Q = reward + \
            self.gamma * \
            max(Q_value)
        return Q

    def backprop(self, x_, reward, next_state):
        ...
        delta = self.cost_derivative(activations[-1], y) * \
            sigmoid_prime(
                zs[-1])
        nabla_b[-1] = delta
        nabla_w[-1] = np.dot(delta, np.asarray(
        ...
        for l in range(2, self.num_layers):
            ...
            sp = sigmoid_prime(
                z)
            delta = np.dot(
                self.weights[-l+1].transpose(), delta) * sp
            nabla_b[-l] = delta
            # print(l, delta.shape, activations[-l-1].shape)
            nabla_w[-l] = np.dot(delta, np.asarray(
    ...
    def save(self, num_epoch, path):
        ...
        np.save(path + 'weight_%d.npy' %
                num_epoch, self.weights)
        np.save(path + 'bias_%d.npy' %
                num_epoch, self.biases)
        return
